...
const Pagination = () => {
    let f =[]
for (let i = 1; i < 5; i++) {
   f.push(<TouchableOpacity style={{width:40,height:40,margin:6,justifyContent:"center",alignItems:"center",flex:1}}><Text style={{padding:10,backgroundColor:"red",color:"#fff"}}>{i}</Text></TouchableOpacity>)
}

    return (
        <View style={{ width: PARAMS.SCREEN_WIDTH ,flexDirection:"row"}}>{f}</View>
    )
}
